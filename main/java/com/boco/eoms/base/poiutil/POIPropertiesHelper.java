 ...
 * @since JDK：1.6
 */
public class POIPropertiesHelper {
    private static final transient Logger log = Logger.getLogger(POIPropertiesHelper.class);

    /**
     * 常用配置属性文件名称.
     */
    private final static String PROPERTIES_FILENAME = "poiexport.properties";
    /**
     * 配置属性对象静态化
     */
    ...
    public static Properties getProperties() {
        if (properties == null) {
            synchronized (POIPropertiesHelper.class) {
                if (properties == null) {
    ...
    public static void loadProperties(String filename) {
        ...
        if (in == null) {
            in = POIPropertiesHelper.class.getResourceAsStream(filename);
            if (in == null) {
